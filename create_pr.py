...
payload = {"title": "Fix for issue #106", "body": "Closes #106\n\nImplemented automated fix.", "head": "KartavyaDikshit:fix-issue-106", "base": "main"}
req = urllib.request.Request("https://api.github.com/repos/Robbo-lab/mcp-task-app/pulls", data=json.dumps(payload).encode(), headers={'Authorization': f'token {token}', 'Accept': 'application/vnd.github.v3+json', 'Content-Type': 'application/json'}, method='POST')
try:
    with urllib.request.urlopen(req, context=ctx) as r: 
        print("[+] PR Created:", json.loads(r.read())['html_url'])
        os.chdir("..")
        shutil.rmtree("/Users/kartavyadikshit/Projects/Open Source/mcp-task-app_bounty_parallel_15", ignore_errors=True)
